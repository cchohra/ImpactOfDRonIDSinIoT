...
def evaluate(_classification, _reducer, _features, _labels):
    ...
    for name, _fullname, model in MODELS:
        ...
        if len(metrics[_classification][name][_reducer]['accuracy']):
            ...
            print("Done")
            writefile = open("results/metrics.dat", "wb")
...
if __name__ == '__main__':
    ...
    if os.path.isfile("results/metrics.dat"):
        print("Result data already exist")
        print("Read old results ... ", end='')
        readfile = open("results/metrics.dat", "rb")
    ...
    writefile = open("results/metrics.json", "w")
    json.dump(metrics, writefile, indent=4, cls=NumpyEncoder)
    writefile.close()
    print("Done")
